...
```
  ...
  Custom UI  queue and case routes, with <TrueForgeUI layout="dock" /> beside them
```

The subagent row is the one the shell cannot stand in for: `npm run attribute` runs the same
functions in `features/attribution` against every grouped case, so a judge with no harness sees
the attribution the agent would produce, but the fan-out itself is the harness's.
...
**`outreach.draft` enforces it, not only `outreach.send`.** A case under three runs, or one
that has not been attributed, gets a refusal naming which of the two is blocking, and the
button on the case screen is disabled with the same words. Gating only the send would leave a
finished message sitting on screen under a case the SOP says nobody should be acting on yet,
which is the state that gets a rule quietly waived.
...
themed atoms, swappable slots and a layout chosen by a string prop (`sidebar`, `drawer`,
`dock`, `widget`), not a general application framework, so the division of labour is fixed
here rather than left to the week:

- **The screens are ordinary React routes** in the shell, reading the queue and case through a
  narrow HTTP read API over the same store (`packages/mcp/src/entrypoints/http.ts`). They own
  the layout, the drill-down and the URL, because "every number is clickable" and "link a judge
  to a case" are both routing problems and neither survives being regenerated by a model on
  each turn.
- **The dock mounts `<TrueForgeUI layout="dock">`** against the harness named by
...
- Approve is rendered disabled with the reason next to it: the run count, the missing
  ...
  produces no draft, and a drafted message on screen is what makes a case look ready to act on.
...
field that justifies it. A healthy count. The screen never shows a list of feeds until one is
asked for.
...
Each figure in the totals strip and each line of the apparatus is a button over
`GET /api/ledger?bucket=<bucket>[&reason=<reason>]`, which reads the run file and returns the
rows behind that count: the feed, the provider, what was seen, the URL, and for a suppressed
row the catalog field and value that answered it. The buckets are `checked`, `healthy`,
`failing`, `actionable`, `suppressed_catalog` and `suppressed_credential`, and they reconcile
by construction: healthy plus failing is checked, actionable plus catalog-suppressed is
failing. `packages/mcp/tests/ledger.test.ts` asserts both identities against the committed run,
so the strip cannot drift from the rows it claims to summarise.
